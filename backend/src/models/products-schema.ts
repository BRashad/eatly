...
export const userScanHistoryTable = pgTable(
  ...
  (table) => ({
    ...
    userScannedAtIdx: index("idx_user_scan_history_user_scanned_at").on(
      table.userId,
 ...
 */
export const ingredientAnalysisTable = pgTable(
  "ingredient_analysis",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    name: text("name").notNull().unique(),
    commonNames: text("common_names").array().notNull().default([]), // Alternative names
    riskLevel: text("risk_level", { enum: ["HIGH", "MEDIUM", "LOW"] })
      .notNull()
      .default("LOW"), // 'HIGH', 'MEDIUM', or 'LOW'
  ...
  (table) => ({
    ...
    riskLevelIdx: index("idx_ingredient_analysis_risk_level").on(
      table.riskLevel,
    ),
  }),
);
...
export const productIngredientsTable = pgTable(
  ...
  (table) => ({
    productIdIdx: index("idx_product_ingredients_product_id").on(
      table.productId,
    ),
    ingredientNameIdx: index("idx_product_ingredients_ingredient_name").on(
