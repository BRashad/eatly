...
import { HEALTH_SCORE_CONFIG } from "@constants/health-score-config";
...
class ProductDataPipeline {
  ...
  async fetchAndStoreProduct(barcode: string): Promise<ProductDetail | null> {
    try {
      ...
      externalProduct.warnings = await this.analyzeIngredientsForWarnings(
      ...
      throw new Error(
        `Failed to fetch and store product: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private async fetchFromMultipleSources(
    barcode: string,
  ): Promise<CreateProductData | null> {
    // Try Open Food Facts first (most comprehensive)
    try {
      const offProduct =
        await externalApiService.fetchProductByBarcode(barcode);
      if (offProduct) {
      ...
      logError(
        "Open Food Facts API error while fetching product",
        error instanceof Error ? error : new Error(String(error)),
        {
          barcode,
          source: "openfoodfacts",
  ...
  private calculateHealthScore(product: CreateProductData): number | undefined {
    ...
    let score: number = HEALTH_SCORE_CONFIG.baseScore;

    product.ingredients.forEach((ingredient) => {
      const ingredientLower = ingredient.toLowerCase();

      // Check for concerning ingredients
      const isConcerning = HEALTH_SCORE_CONFIG.concerningIngredients.some(
        (concerning) => ingredientLower.includes(concerning),
      );

      // Check for beneficial ingredients
      const isBeneficial = HEALTH_SCORE_CONFIG.beneficialIngredients.some(
        (beneficial) => ingredientLower.includes(beneficial),
      );
    ...
    if (product.nutritionInfo) {
      ...
      // High sodium penalty
      if (sodium && sodium > HEALTH_SCORE_CONFIG.thresholds.sodium) {
        // mg per serving
      ...
      if (
        saturatedFat &&
        saturatedFat > HEALTH_SCORE_CONFIG.thresholds.saturatedFat
      ) {
        // g per serving
        score = Math.max(1, score - 1);
      }
      ...
      // High sugars penalty
      if (sugars && sugars > HEALTH_SCORE_CONFIG.thresholds.sugars) {
        // g per serving
        score = Math.max(1, score - 1);
      }

      // Low calories bonus
      if (calories && calories < HEALTH_SCORE_CONFIG.thresholds.lowCalories) {
        // per serving
   ...
   */
  private async analyzeIngredientsForWarnings(
    ingredients: string[],
  ): Promise<string[]> {
    ...
    const warningIngredients: Record<string, string> = {
      ...
      "high fructose corn syrup":
        "Contains refined sugar - may impact metabolism",
    ...
    ingredients.forEach((ingredient) => {
      ...
      for (const [allergen, warning] of Object.entries(allergenMap)) {
        if (
          ingredientLower.includes(allergen) &&
          !warnings.find((w) => w.includes("allergic"))
        ) {
      ...
      for (const [ingredientName, warning] of Object.entries(
        warningIngredients,
      )) {
   ...
   */
  async importProductsBySearch(
    searchTerms: string[],
    limitPerTerm: number = 10,
  ): Promise<{
    imported: number;
    duplicates: number;
    errors: number;
    ...
    for (const searchTerm of searchTerms.slice(0, 5)) {
      ...
      try {
        const searchResult = await externalApiService.searchProducts(
          searchTerm,
          1,
          limitPerTerm,
        );
          ...
          for (const product of searchResult.products) {
            try {
              ...
              results.details.push(
                `Failed to import ${product.barcode}: ${
        ...
        results.errors++;
        results.details.push(
          `Failed to search "${searchTerm}": ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
